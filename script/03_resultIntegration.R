...
control.seuobj.singlet$sum.v2[rownames(control2125.final.meta)] <- as.character(control2125.final.meta$sum.v2)
...
Idents(control.seuobj.singlet) <- control.seuobj.singlet$sum.v2
final.markers.sum.v2 <- FindAllMarkers(control.seuobj.singlet,
                                   only.pos = T)
save(final.markers.sum.v2,file = paste0(output_dpath,prefix,
                                    "final.markers.sum.v2.RData"))

top20 <- final.markers.sum.v2 %>%
  group_by(cluster) %>%
...
Top20Heatmap <- DoHeatmap(control.seuobj.singlet,
                          ...
                          angle = 90,
                          size = 3) + mytheme
ggsave(filename = paste0(output_fpath,prefix,"2_Top20Heatmap.control.singlet.sum.v2.pdf"),
       plot = Top20Heatmap,
