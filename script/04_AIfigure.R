...
plot2 <- DotPlot(control.seuobj.singlet,features = Allcellfeatures,
  ...
  mytheme
ggsave(filename = paste0(output_path,prefix,"3_DotPlot_allcellfeature.pdf"),plot = plot2,
...
ggsave(filename = paste0(output_path,prefix,"4_FeaturePlot_stemcell_v2.pdf"),plot = plot3,
       device = "pdf",height = height,width = width*3/2,units = "mm")
...
plot4 <- DimPlot(Bcells,label = T,label.box = T,
                 ...
                 repel = T) + mytheme
ggsave(filename = paste0(output_path,prefix,"/4_DimPlot_Bcell.pdf"),plot = plot4,
...
ggsave(filename = paste0(output_path,prefix,"5_FeaturePlot_Bcell.pdf"),plot = plot5,
       device = "pdf",height = height,width = width*3/2,units = "mm")
...
plot6 <- df.cellratio %>% ggbarplot(x = "Method",
  ...
  mytheme +
  theme_base()
ggsave(filename = paste0(output_path,prefix,"6_ggbarplot_ratio.pdf"),plot = plot6,
...
plot7 <- DoHeatmap(control.seuobj.singlet, features = top20$gene,
  ...
  NoLegend() + mytheme
ggsave(filename = paste0(output_path,prefix,"7.2_DoHeatmap.pdf"),plot = plot7,
