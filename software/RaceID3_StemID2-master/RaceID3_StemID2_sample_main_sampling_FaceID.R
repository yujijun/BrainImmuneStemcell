##------ Update Time:Wed Oct 20 11:18:23 2021 ------##
#### 01_load all library and hyperpaarameter ####
source("RaceID3_StemID2_class.R")
library(tidyverse)
library(Seurat)
work_path <- "/mnt/data/yjj/BrainImmune/software/RaceID3_StemID2-master"
setwd(work_path)
output_path = "RaceID3_1019/"
outminc.use=10
outlg.use=10
pdishuf.use = 10000
...
# save(choosen_cell,file = "./RaceID3_1019/choosen_cell.RData")


#### 04_Create SCseq object and clustering ####
load("./RaceID3_1019/choosen_cell.RData")
control.choosen <- subset(control.seuobj.singlet,cells = choosen_cell)
x <- control.choosen@assays$RNA@counts
...
scLabel <- control.choosen@meta.data[colnames(sc@fdata),]$sum.v2
...
dev.off()
#### 05_Calculation of StemID2 score ####
# initialization
ltr <- Ltree(sc)
...
#### 06_Output cluster result #### 
cpart <- sc@cpart
anno <- control.seuobj.singlet$sum.v2[names(sc@cpart)]
cluster <- data.frame(
    cpart = cpart,
    anno = anno
)
save(cluster, file = paste0("./software/RaceID3_StemID2-master/RaceID3_1019/cluster.RData"))
